import { pgSchema, pgEnum, pgTable, uuid, text, timestamp, varchar, boolean, jsonb, date, index, uniqueIndex, foreignKey, primaryKey, unique } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const neonAuth = pgSchema("neon_auth");
export const status = pgEnum("status", ["Applied", "Recruiter Screen", "Initial Interview", "Technical Interview", "Final Interview", "Offer", "Rejected", "Withdrawn"])
...
export const application = pgTable("application", {
	...
	dateApplied: date("date_applied").default(sql`now()`).notNull(),
